...
interface EventRow { id: number; url: string | null; canonical_url: string | null; type: string; tab_id: number | null; ts: number }
interface PageRow { canonical_url: string; url: string | null; title: string | null; description: string | null; trail_id: string | null }
...
async function main(): Promise<void> {
  ...
  // ---- 1. auth-flow pages ----
  const pages = db.prepare('SELECT canonical_url, url, title, description, trail_id FROM pages').all() as unknown as PageRow[];
  const authPages = pages.filter((p) => isSensitiveUrl(p.url || p.canonical_url));
  log(`  auth-flow pages to remove ........ ${authPages.length}`);
  for (const p of authPages) log(`      - ${(p.title || '').slice(0, 46).padEnd(46)} ${(p.url || '').slice(0, 60)}`);

  // ---- 2. secret params ----
  const events = db.prepare('SELECT id, url, canonical_url, type, tab_id, ts FROM events').all() as unknown as EventRow[];
  ...
  const redactPages = pages.filter((p) => !isSensitiveUrl(p.url || p.canonical_url) && p.url && redact(p.url) !== p.url);
  ...
  const titleRows = db.prepare(
    "SELECT id, title FROM events WHERE title IS NOT NULL AND title LIKE '%=%'",
  ).all() as unknown as { id: number; title: string }[];
  const badTitles = titleRows.filter((r) => redactTextParams(r.title) !== r.title);
  const pageTitleRows = db.prepare(
    "SELECT canonical_url, title FROM pages WHERE title IS NOT NULL AND title LIKE '%=%'",
  ...
  for (const e of events) {
    ...
    if (e.type === 'close') { tabCanon.delete(tab); continue; }
    if (!canon) continue;
    const moved = tabCanon.get(tab) !== canon;
    tabCanon.set(tab, canon);
    const seen = lastSeen.get(canon);
    if (seen === undefined) { recomputed.set(canon, 1); lastSeen.set(canon, e.ts); continue; }
    if (e.type === 'navigate' && moved && e.ts > seen) recomputed.set(canon, (recomputed.get(canon) ?? 1) + 1);
    lastSeen.set(canon, Math.max(seen, e.ts));
  }

  const current = db.prepare('SELECT canonical_url, visit_count FROM pages').all() as unknown as { canonical_url: string; visit_count: number }[];
  const drops = current
    .map((c) => {
  ...
  for (const d of drops.slice(0, 8)) log(`      ${String(d.from).padStart(5)} -> ${String(d.to).padEnd(4)} ${d.url.slice(0, 62)}`);
  ...
  try {
    ...
    for (const p of authPages) db.prepare('DELETE FROM checkpoint_pages WHERE canonical_url = ?').run(p.canonical_url);
    ...
    for (const p of redactPages) {
      ...
      if (newCanon !== p.canonical_url) {
        ...
        if (clash) { delPage.run(p.canonical_url); continue; }
      }
      db.prepare('UPDATE pages SET url = ?, canonical_url = ? WHERE canonical_url = ?')
        .run(u, newCanon, p.canonical_url);
    }
    ...
    for (const row of db.prepare('SELECT canonical_url FROM pages').all() as unknown as { canonical_url: string }[]) {
    ...
    for (const trailId of touchedTrails) {
      const rows = db.prepare('SELECT title, description, url, canonical_url FROM pages WHERE trail_id = ?')
