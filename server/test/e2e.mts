...
const daemon = spawn(process.execPath, [join(REPO, 'server/dist/index.js')], {
  env,
...
async function api(path: string, init?: { method?: string; body?: unknown; token?: string }) {
  const r = await fetch(`http://127.0.0.1:${PORT}${path}`, {
    ...
    headers: {
      ...
      ...(init?.token ? { 'x-tabzero-token': init.token } : {}),
    },
...
check(
  !!health.token,
  'GET /health hands out a token (extension bootstrap path)',
  `len ${health.token?.length}`,
);
check(
  health.token === readFileSync(join(DATA, 'token'), 'utf8').trim(),
  'token matches the 0600 file on disk',
);
...
visit(
  ...
  'Write-Ahead Logging',
  'SQLite WAL mode allows readers and one writer to proceed concurrently without blocking each other.',
);
visit(
  102,
  1,
  ...
  'How SQLite implements file locking, rollback journals, and the transition to WAL journalling.',
  101,
);
visit(
  103,
  ...
  'PRAGMA busy_timeout',
  'Set how long a connection waits for a lock before returning SQLITE_BUSY.',
);
visit(
  104,
  1,
  'https://kerkour.com/sqlite-for-servers',
  'SQLite Concurrency Tuning For Servers',
  'Practical settings for using SQLite under concurrent server load: WAL, busy timeout, synchronous NORMAL.',
);
// Trail B — unrelated shopping (3 pages) so clustering has to separate them
visit(
  201,
  2,
  'https://baristahustle.com/blogs/barista-hustle/grind-size-distribution',
  'Grind Size Distribution',
  'Why particle distribution matters more than average grind size for espresso extraction.',
);
visit(
  202,
  2,
  'https://www.seattlecoffeegear.com/products/df64-grinder',
  'DF64 Single Dose Espresso Grinder',
  'Flat burr single dose grinder for espresso, 64mm burrs, low retention.',
);
visit(
  203,
  2,
  'https://home-barista.com/grinders/df64-review-t12345.html',
  'DF64 Grinder Long Term Review',
  'Long term impressions of the DF64 espresso grinder, burr alignment and retention.',
);

console.log('\n## 2. ingest (POST /events, exactly the extension payload)');
const ing = await api('/events', { method: 'POST', token: TOKEN, body: { events: evs } });
check(
  ing.json?.ok === true && ing.json?.count === evs.length,
  'all events accepted',
  `${ing.json?.count}/${evs.length}`,
);
...
check(
  pages.every((p) => p.visit_count === 1),
  'duplicate navigate ticks did NOT inflate visit_count',
...
const espresso = pages.filter(
  (p) => !p.canonical_url.includes('sqlite.org') && !p.canonical_url.includes('kerkour'),
);
...
console.log(
  `    espresso pages split across ${espressoTrails.size} trail(s): ${[...espressoTrails].join(', ')} — lexical clustering, no embeddings`,
);
check(
  pages.find((p) => p.canonical_url.includes('lockingv3'))!.trail_id === trailA,
...
for (let i = 0; i < 40 && !labelled; i++) {
  await sleep(5000);
  const rows = q(
    'SELECT id,label,one_liner,category,label_dirty,summary,summary_source FROM trails WHERE label_dirty = 0 AND one_liner IS NOT NULL',
  );
...
for (let i = 0; i < 30 && !pushed; i++) {
  const rows = q(
    'SELECT id,engram_dirty,engram_ref,last_engram_push FROM trails WHERE engram_dirty = 0 AND engram_ref IS NOT NULL',
  );
  if (rows.length >= 2) pushed = rows;
...
if (!memory) {
  ...
  check(
    src === 'engram',
    'cached recap upgraded to summary_source=engram once the memory existed',
    `source=${src}`,
  );
  const served = String(d.json?.summary ?? '').trim();
  check(
    served === memory.content.trim(),
    'the served recap IS the Engram memory, not a local placeholder that merely looks similar',
    served === memory.content.trim()
      ? ''
      : `served ${served.length}ch vs memory ${memory.content.trim().length}ch`,
  );
...
check(
  r1.json?.urls?.length === 3,
  'returns exactly the 3 checkpointed tabs, not all 4',
  `${r1.json?.urls?.length}`,
);
...
check(
  r2.json?.urls?.length === 4,
  'a page visited after the checkpoint joins the reopen set',
  `${r2.json?.urls?.length}`,
);
check(
  r2.json.urls.some((u: string) => u.includes('howtocorrupt')),
  'the new page is specifically included',
);

console.log('\n## 8. read surfaces');
const week = await api('/week', { token: TOKEN });
check(
  Array.isArray(week.json?.stats) && week.json.stats.length > 0,
  'week stats render',
  `${week.json?.stats?.length} stats`,
);
check(!('emoji' in (week.json.stats[0] ?? {})), 'no dead emoji field on stats');
const search = await api('/search', {
  method: 'POST',
  token: TOKEN,
  body: { query: 'sqlite locking under concurrent writes' },
});
check(
  (search.json?.hits?.length ?? 0) > 0,
  'semantic/keyword search finds the trail',
  `${search.json?.hits?.length} hits, why=${search.json?.hits?.[0]?.why}`,
);
...
for (const qq of paraphrases) {
  ...
  console.log(
    `    "${qq}" -> ${(r.json?.hits ?? []).map((h: any) => h.trail.id + ':' + h.why).join(', ') || 'no hits'}`,
  );
  if (s2 && !semantic) semantic = { q: qq, hit: s2 };
}
check(
  !!semantic,
  'the Engram semantic branch contributed a hit keyword could not',
